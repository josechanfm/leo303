      ...
      <div class="bg-white relative shadow  p-5 rounded-lg overflow-x-auto">
        ...
        >
            <a-form-item :label="$t('organization')" name="organization_id">
                <a-select v-model:value="form.organization_id" :options="organizations" :fieldNames="{value:'id',label:'name_zh'}"/>
            </a-form-item>
            ...
            <a-form-item :label="$t('banner_image')" name="banner_image">
              {{ form.media }}
                <div v-if="form.media.length">
                    <inertia-link
                    :href="route('admin.form.deleteMedia', form.media[0].id)"
...
export default {
  ...
  methods: {
    ...
    updateRecord() {
        ...
        this.$inertia.post(
            route("admin.forms.update", this.form.id),this.form,{
                onSuccess: (page) => {
                    console.log(page);
                },
                onError: (error) => {
